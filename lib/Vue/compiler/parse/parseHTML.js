...
export default function parseHTML(html, options) {
  ...
  while (html) {
     ...
     * 而这一步, 我们的目的是为了确保textEnd是从左开始, 最近的一个节点的<的下标值!
     * 这样, 我们就可以通过html.substring(0, textEnd)来拿到真正的文本部分
     */
    let text
    if (textEnd >= 0) {
      /**
       * 注意下面这个rest, 根据上面我们说的2种情况, 我们可以推断出它的值
      ...
      let next
      /**
       * 如果rest不是<div或者</div>这种形式, 这样, 第1,2种情况第一次循环都不会进入
       * 而首次能进入的, 只有3,4种情况了
       */
      while (!rest.match(startTagOpen) && !rest.match(endTag)) {
        ...
        // 此时再更新rest
        // 此时第3种情况都变成了</div>
        // 此时第4种情况都变成了<56</div>, 成为了之前第三种情况的样子, 所以继续while循环
        rest = html.slice(textEnd)
        // 如果还有
      }
      // 这样, 我们就能用表示首个节点的<的下标值, 来获取文本的部分
